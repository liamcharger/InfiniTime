...
namespace {
  ...
  void GetSleepInfoCallback(lv_obj_t* btn, lv_event_t event) {
    if (event == LV_EVENT_CLICKED) {
      // Get sleep info
      auto* screen = static_cast<Sleep*>(lv_obj_get_user_data(btn));
      screen->GetSleepInfo(screen->ReadDataCSV("Sleep_Data.csv"));
    }
  }
...
Sleep::Sleep(Controllers::HeartRateController& heartRateController, Controllers::DateTime& dateTimeController, Controllers::FS& fsController, System::SystemTask& systemTask)
  ...
  std::vector<std::tuple<int, int, int, int, int>> data = ReadDataCSV("Sleep_Data.csv");
...
double Sleep::ConvertToMinutes(int hours, int minutes, int seconds) {
  return hours * 60 + minutes + seconds / 60.0;
}

// Get the moving average of BPM Values
std::vector<double> Sleep::MovingAverage(const std::vector<int>& bpmData, int windowSize) {
...
std::vector<std::pair<double, double>> Sleep::DetectSleepRegions(const std::vector<double>& bpmData, const std::vector<double>& time, double threshold) {
...
// Get Sleep Info
void Sleep::GetSleepInfo(std::vector<std::tuple<int, int, int, int, int>> data) {
...
void Sleep::GetBPM() {
  ...
  WriteDataCSV("Sleep_Data.csv", data);
...
*/
void Sleep::WriteDataCSV(const char* fileName, const std::vector<std::tuple<int, int, int, int, int>>& data) {
  lfs_file_t file;
...
// Read data from CSV
std::vector<std::tuple<int, int, int, int, int>> Sleep::ReadDataCSV(const char* filename) {
  lfs_file_t file;
...
// Clear data in CSV
void Sleep::ClearDataCSV(const char* filename) {
  lfs_file_t file;
